...
private fun filter(nums: List<Int>, predicate: (Int) -> Boolean): List<Int> {
    val list = mutableListOf<Int>()
    for (num in nums) {
        if (predicate(num)) {
            list += num
...
private fun filterNegative(nums: List<Int>): List<Int> {
    val list = mutableListOf<Int>()
    for (n in nums) {
        if (0 > n) {
            list += n
...
private fun filterNotNegative(nums: List<Int>): List<Int> {
    val list = mutableListOf<Int>()
    for (n in nums) {
        if (0 <= n) {
            list += n
